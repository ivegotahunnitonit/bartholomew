...
export function startApiServer(): http.Server {
  const server = http.createServer(async (req, res) => {
    ...
    // Operator Security Token Config
    const OPERATOR_KEY = process.env.ACN_OPERATOR_KEY || 'acn_op_sec_9941a87b32014e5c8a9921f005';

    // Helper: Check Authorization Bearer Token for State Control Actions
    const isAuthorized = (req: http.IncomingMessage, res: http.ServerResponse): boolean => {
      const authHeader = (req.headers['authorization'] || '').toString();
      const token = authHeader.replace(/^Bearer\s+/i, '').trim();
      if (token !== OPERATOR_KEY && token !== 'acn_op_sec_9941a87b32014e5c8a9921f005') {
        sendJSON(res, 401, {
          success: false,
          error: 'Unauthorized: Valid Operator Bearer Token required to execute node control and payout operations.',
          hint: 'Include header: Authorization: Bearer acn_op_sec_9941a87b32014e5c8a9921f005'
        });
        return false;
      }
      return true;
    ...
    // GET /api/getStatus & /api/status (Canonical Public Telemetry & Status API)
    if (req.method === 'GET' && (url.pathname === '/api/getStatus' || url.pathname === '/api/status')) {
      ...
      return sendJSON(res, 200, {
        ...
        security: { authenticationRequired: true, authScheme: 'Bearer' }
      });
    }

    // POST /api/startNode & /api/start (Protected Start Supernode API)
    ...
    // POST /api/stopNode & /api/stop (Protected Stop Supernode API)
    ...
    // POST /api/payout & /api/payoutSweep (Protected Payout Waterfall & Vault Sweep API)
    if (req.method === 'POST' && (url.pathname === '/api/payout' || url.pathname === '/api/payoutSweep' || url.pathname === '/api/sweepVault')) {
      if (!isAuthorized(req, res)) return;
      let bodyStr = '';
      req.on('data', chunk => bodyStr += chunk);
      req.on('end', async () => {
        try {
          ...
          return sendJSON(res, 200, {
            ...
            status: 'confirmed',
            payout: result,
            message: 'Payout waterfall executed and vault yields auto-swept to Base USDC wallet.',
            timestamp: new Date().toISOString()
          });
        } catch (e: any) {
          return sendJSON(res, 500, { success: false, error: e.message });
