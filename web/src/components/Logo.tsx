...
export function BartholomewLogo({ size = 30, className = '' }: { size?: number; className?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <defs>
        <linearGradient id="shieldOledGrad" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#f59e0b" />
          <stop offset="50%" stopColor="#10b981" />
          <stop offset="100%" stopColor="#059669" />
        </linearGradient>
        <filter id="shieldOledGlow" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="1" stdDeviation="2" floodColor="#f59e0b" floodOpacity="0.3" />
        </filter>
      </defs>
      {/* Shield Outer Body with Amber-Emerald Gradient */}
      <path
        d="M18 3L5 8.5V17C5 25.5 10.8 30.8 18 33C25.2 30.8 31 25.5 31 17V8.5L18 3Z"
        fill="url(#shieldOledGrad)"
        filter="url(#shieldOledGlow)"
      />
      {/* Inner Pitch Black Shield Overlay */}
      <path
...
export default function Logo({ size = 30, className = '', showText = true, textSize = 'text-sm' }: LogoProps) {
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      <BartholomewLogo size={size} />
      {showText && (
        <span
          className={`font-bold tracking-tight text-white font-sans ${textSize}`}
        >
          Bartholomew
        </span>
      )}
